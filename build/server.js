...
function globalErrorHandler(error2, _, reply) {
  ...
  if (error2 instanceof import_zod10.ZodError) {
    return reply.status(400).send({ message: "Validation Error", errors: error2.format() });
  }
  if (error2 instanceof import_pg2.DatabaseError) {
    return reply.status(400).send({ message: "Integrity Error", errors: error2.message });
  }
  if (error2 instanceof ResourceNotFoundError) {
    return reply.status(404).send({ message: error2.message });
  }
  if (error2 instanceof InvalidCredentialError) {
    return reply.status(401).send({ message: error2.message });
  }
}
...
app.listen({
  ...
  console.error(`Error starting server on port ${env.PORT}, ${import_console.error}`);
});
